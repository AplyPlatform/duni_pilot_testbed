...
function moveFlightHistoryMapAndCada(lat, lng, cada) {
	...
	$('#dataTable-Flight_list').append("<div class='text-center'><h4>이 지역을 드론으로 촬영한 영상을 보려면?</h4><a class='btn btn-primary btn-sm' role='button' href='https://duni.io/index.php?page=rental' target='_new' onClick='GATAGM(\"util_request_duni_btn\",\"SERVICE\",\"" + latlng + "\");'>드론촬영 요청</a></div>");
...
function setAreaInfo(ainfo) {
	...
	area_infos.forEach(function (ai) {
		...
		let areaVec = ai.arrayvec;
		let _area_polyline = new ol.Feature({ geometry: new ol.geom.LineString(areaVec) });
