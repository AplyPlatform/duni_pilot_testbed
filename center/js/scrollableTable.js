...
var scrollableTable = function(id, wrapperId, enableLogging=false) {
    ...
    let logging = enableLogging;
    let root = this;
    let lastRowId = 0;
    let enabledSingleSelect = true;
    let metadata = {};
    ...
    var initiallyPrepareMetaData = function(data, metadataRowInfo, parentId, level, nestedIds) {
        ...
        $.each(data, function(index) {
            ...
            let rowId = id+'_rowId_'+ (++lastRowId);
            let parentRowId = id+'_rowId_'+parentId;
            ...
            if (metadata.subtreePropertyName.length > 0 &&
                ...
                let dataSubtree = data[index][metadata.subtreePropertyName];
                let rowInfoSubtree = rowInfo.rowInfoSubtree;
    ...
    var selectPreviousRow = function() {
        ...
        // if (metadata.selectedRows.length == 1) {
            let rowInfo = metadata.selectedRows[0];
            let firstRow = $('#'+id+' > tbody > tr:first');
